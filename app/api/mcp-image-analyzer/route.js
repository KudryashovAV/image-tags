...
async function analyzeImagesWithGPT(imagesChunk, finalConfig) {
  ...
  // Системный промпт с жестким структурированием и принудительным длинным примером
  const systemPrompt = `Ты — профессиональный арт-директор и эксперт по составлению гипердетализированных, объемных промптов для генераторов изображений (gpt-image-2, gemini-3-pro-image).
