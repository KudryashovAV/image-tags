...
import { getCrashesDataOnly } from "../../services/googlePlayVitals";
...
export async function GET(request) {
  ...
  try {
    ...
    const fatalData = await getCrashesDataOnly();

    // 2. Проверка структуры ответа (на случай, если вернулся null/undefined)
    currentStep = "Проверка валидности полученных данных fatalData";
    if (!fatalData) {
    ...
    return NextResponse.json(fatalData, {
      status: 200,
      headers: {
